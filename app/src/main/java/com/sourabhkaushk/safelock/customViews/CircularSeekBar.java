...
public class CircularSeekBar extends View {
    ...
    private void calculateProgressDegrees() {
        ...
        mProgressDegrees = (mProgressDegrees < 0 ? Math.abs(mProgressDegrees) < 55f && wasZero ?  0f : 360f + mProgressDegrees : mProgressDegrees); // Verified
        if(mNegativeEnabled&&mProgress>=-5f && mProgress<=5f){
            wasZero=true;
        }else{
            wasZero=false;
        }
...
//            }
    ...
    public void setNegativeEnabled(boolean negativeEnabled) {
        ...
        wasZero=false;
        mIsInNegativeHalf=negativeEnabled;
    }
...
//                    mLockAtStart = false;
            ...
            case MotionEvent.ACTION_MOVE:
                if (mUserIsMovingPointer) {
                    ...
                    if (progressNearZero && mNegativeEnabled) {   // logic for negative flip
//                        if (touchOverEnd)
//                            mIsInNegativeHalf = false;
...
//                        mLockAtStart=true;
//                        }
                    } else if (progressNearMax) {  // logic for end lock.
